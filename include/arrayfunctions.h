...
INLINE void ArrayCheckEqual(const char *msg, const double *var_adj, const double *var_sep, int npoints, int nvars)
{
    double max_err = 0.0;
    for (int j=0; j<npoints; j++) {
        for (int v=0; v<nvars; v++) {
            if (var_sep[j+v*npoints] != var_adj[j*nvars+v]) {
                double diff = fabs(var_sep[j+v*npoints]-var_adj[j*nvars+v]);
                max_err = (max_err < diff) ? diff : max_err;
            }
        }
    }

    if (max_err > 1e-10) {
        printf("ArrayCheckEqual: %-30s max_err = %e\n", msg, max_err);
        exit(0);
    }
    return;
}

INLINE void ArrayCheckEqual2(const char *msg, const double *var_adj, const double *var_sep, int npoints)
{
    double max_err = 0.0;
    for (int j=0; j<npoints; j++) {
        if (var_sep[j] != var_adj[j]) {
            double diff = fabs(var_sep[j]-var_adj[j]);
            max_err = (max_err < diff) ? diff : max_err;
        }
    }
