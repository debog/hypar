 ...
 * format. The difference is written out in the binary format
 * as well.
 *
 * If the size of the solution and the reference differ, they
 * must differ by an integer power of 2 along each dimension.
...
int main()
{
  ...
  char  sol_fname[_MAX_STRING_SIZE_],
        ref_fname[_MAX_STRING_SIZE_];

  FILE* in;
  in = fopen("compare.inp","r");
  fscanf(in,"%s", sol_fname);
  fscanf(in,"%s", ref_fname);
  fclose(in);
  ...
  SolObj reference_int;
  createSolObj( solution.ndims,
                solution.nvars,
                solution.size,
                &reference_int );

  ierr = InterpolateGlobalnDVar(reference_int.size,
                                reference_int.u,
                                reference.size,
                                reference.u,
                                reference_int.nvars,
                                reference_int.ndims,
                                reference_int.is_periodic );
  if (ierr) return ierr;
  deleteSolObj(&reference);

  double diff_norm_arr[3];
  ierr = ComputeDiffNorm( &solution,
                          &reference_int,
                          diff_norm_arr );
  if (ierr) return ierr;

  writeBinary(  reference_int.ndims,
                reference_int.nvars,
                reference_int.size,
                solution.x,
                reference_int.u,
