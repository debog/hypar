...
Location: \b hypar/Examples/1D/LinearAdvection/SineWave_libROM
          (This directory contains all the input files needed
          to run this case.)
...
Reduced Order Modeling:
 + Type: Dynamic Mode Decomposition (DMD)
...
subdirectory does not exist, the code will \b not report an error; the DMD
...
Reduced Order Modeling:
 + Type: Dynamic Mode Decomposition (DMD) with time windowing
 + Latent subspace dimension: 16 (DMDROMObject::m_rdim)
 + Number of samples per time window: 100 (DMDROMObjects::m_num_window_samples)
