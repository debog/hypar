'''
Python script to compute the difference between the HyPar and
libROM solutions and plot it.
...
if solver_inp_data['op_overwrite'] == 'no':
  ...
  for var in range(nvars):
    for i in range(n_snapshots):
      ...
      for s in range(nsims):
        sol_fom_snapshots_sim = sol_fom_snapshots[s*n_snapshots:(s+1)*n_snapshots]
        sol_rom_snapshots_sim = sol_rom_snapshots[s*n_snapshots:(s+1)*n_snapshots]
        sol_fom_2d = np.transpose(sol_fom_snapshots_sim.reshape(n_snapshots,size[1],size[0],nvars))[var,:,:,i]
        sol_rom_2d = np.transpose(sol_rom_snapshots_sim.reshape(n_snapshots,size[1],size[0],nvars))[var,:,:,i]
        ...
        # Plot solutions
        plot_diff = axes.pcolor(x2d, y2d, sol_diff_2d, cmap=colormap)
        axes.set_title('Diff: var {:}, t={:.3}, norm={:.3}'.format(var,i*dt_snapshots,diff_norm))
        fig.colorbar(plot_diff, ax=axes)
        if nsims > 1:
          plt_fname = plt_dir_name+'/fig_diff_'+f'{s:02d}'+'_'+f'{var:02d}'+'_'+f'{i:05d}'+'.png'
        else:
  ...
  for var in range(nvars):
    ...
    for s in range(nsims):
      sol_fom_snapshots_sim = sol_fom_snapshots[s*n_snapshots:(s+1)*n_snapshots]
      sol_rom_snapshots_sim = sol_rom_snapshots[s*n_snapshots:(s+1)*n_snapshots]
      sol_fom_2d = np.transpose(sol_fom_snapshots_sim.reshape(size[1],size[0],nvars))[var,:,:]
      sol_rom_2d = np.transpose(sol_rom_snapshots_sim.reshape(size[1],size[0],nvars))[var,:,:]
      sol_diff_2d = sol_fom_2d - sol_rom_2d
      fom_sol_norm = LA.norm(sol_fom_2d)
      diff_norm = LA.norm(sol_diff_2d)
      if fom_sol_norm > 1e-14:
      ...
      # Plot solutions
      plot_diff = axes.pcolor(x2d, y2d, sol_diff_2d, cmap=colormap)
      axes.set_title('Diff: var {:}, t={:.3}, norm={:.3}'.format(var,t_final,diff_norm))
      fig.colorbar(plot_diff, ax=axes)
      if nsims > 1:
        plt_fname = plt_dir_name+'/fig_diff_'+f'{s:02d}'+'_'+f'{var:02d}'+'.png'
      else:
