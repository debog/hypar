...
def plotData( data_3d: np.ndarray,
              x3d: np.ndarray,
              y3d: np.ndarray,
              z3d: np.ndarray,
              fig_filename,
  ...
  kw1 =  {'vmin': data_3d.min(),
          ...
          'levels': np.linspace((data_3d.min()+0.1*(data_3d.max()-data_3d.min())),
                                (data_3d.max()-0.1*(data_3d.max()-data_3d.min())),
                                25),
          'antialiased' : True,
          'alpha' : 0.8,
          'cmap' : colormap,
        }
  kw2 =  {'vmin': data_3d.min(),
          'vmax': data_3d.max(),
          'levels': np.linspace(data_3d.min(),
                                data_3d.max(),
                                100),
          'antialiased' : True,
          'alpha' : 0.6,
...
if solver_inp_data['op_overwrite'] == 'no':
  ...
  for i in range(n_snapshots):
    for sim in range(nsims):
      ...
      plotData(theta, x3d, y3d, z3d, plt_fname, 'theta', i*dt_snapshots)
  ...
  for sim in range(nsims):
    ...
    plotData(theta, x3d, y3d, z3d, plt_fname, 'theta', t_final)
