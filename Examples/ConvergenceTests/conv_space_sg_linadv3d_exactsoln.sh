...
#################################################################
#
# Bash script to run a spatial convergence test on a smooth 3D linear
# advection problem - evolution of a sine wave.
#
...
# linear advection
export p_adv_speed="1.0 1.0 1.0"

# sparse grids related stuff
export p_imin=3
...
while [ $BC_COUNTER -lt 2 ]; do
  ...
  eval 'export "$varname"="0 0 0 1 0 1"'
  let BC_COUNTER=BC_COUNTER+1
done
BC_COUNTER=2
while [ $BC_COUNTER -lt 4 ]; do
  varname="p_dim_${BC_COUNTER}"
  eval export "$varname"="1"
  varname="p_limits_${BC_COUNTER}"
  eval 'export "$varname"="0 1 0 0 0 1"'
  let BC_COUNTER=BC_COUNTER+1
done
BC_COUNTER=4
while [ $BC_COUNTER -lt $p_nb ]; do
  ...
  varname="p_limits_${BC_COUNTER}"
  eval 'export "$varname"="0 1 0 1 0 0"'
  let BC_COUNTER=BC_COUNTER+1
done
BC_COUNTER=0
while [ $BC_COUNTER -lt $p_nb ]; do
...
echo "linear advection equation (3D):"
...
while [ $REF_LEVEL -lt $num_refinements ]; do
  ...
  export p_size="$NUM_PTS $NUM_PTS $NUM_PTS"
  export p_iproc="$NPROC $NPROC $NPROC"
