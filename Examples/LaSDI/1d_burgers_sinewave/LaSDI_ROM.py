...
t_shock_arr = np.float32(param_data['t_shock'])
P = list(product(t_shock_arr))
P = np.array(P)
...
start = time.time()
FOM_recon = LaSDI_model.generate_ROM(solution_snapshots_FOM[0], np.array([2.0]), t_arr)
...
ax.set_title('Relative Error: {:.3}  Speedup: {:.3} times'.format(LA.norm(FOM_recon[n_snapshots//2]-solution_snapshots_FOM[n_snapshots//2])/LA.norm(solution_snapshots_FOM[n_snapshots//2]), FOM_time/LaSDI_time))
ax.plot(x,solution_snapshots_FOM[n_snapshots//2], label = 'FOM')
ax.plot(x, FOM_recon[n_snapshots//2],'--', label = 'LaSDI')
ax.legend()
ax.set_xlim(np.min(x),np.max(x))
plt.savefig('FOM_vs_LaSDI_solution_preshock.png')
...
ax.legend()
ax.set_xlim(np.min(x),np.max(x))
plt.savefig('FOM_vs_LaSDI_solution_postshock.png')
