                        ...
                        )
{
  ...
  if (param->m_grav_x != 0.0) {
    ...
    IERR NavierStokes2DSourceFunction(SourceC,a_u,x,solver,mpi,a_t,_XDIR_); CHECKERR(ierr);
    ...
    IERR NavierStokes2DSourceUpwind(SourceI,SourceL,SourceR,a_u,_XDIR_,solver,a_t);
  ...
  if (param->m_grav_y != 0.0) {
    ...
    /* calculate the split a_source function exp(-phi/RT) */
    IERR NavierStokes2DSourceFunction(SourceC,a_u,x,solver,mpi,a_t,_YDIR_); CHECKERR(ierr);
    /* calculate the left and right interface a_source terms */
    ...
    /* calculate the final interface a_source term */
    IERR NavierStokes2DSourceUpwind(SourceI,SourceL,SourceR,a_u,_YDIR_,solver,a_t);
                                ...
                                )
{
  ...
  while (!done) {
    ...
    (a_f+_MODEL_NVARS_*p)[3] = param->m_grav_field_g[p];
