                     ...
                     )
{
      ...
      if (!strcmp(word, "begin")){
        while (strcmp(word, "end")){
          ...
          if (!strcmp(word, "gamma")) {
            ...
            ferr = fscanf(in,"%lf",&physics->m_grav);
            if (ferr != 1) return(1);
  ...
  IERR MPIBroadcast_double    (&physics->m_grav     ,1,0,&mpi->m_world);                  CHECKERR(ierr);
  IERR MPIBroadcast_integer   (&physics->m_grav_type,1,0,&mpi->m_world);                  CHECKERR(ierr);
  ...
  if      (!strcmp(physics->m_upw_choice,_ROE_    )) solver->Upwind = Euler1DUpwindRoe;
  else if (!strcmp(physics->m_upw_choice,_RF_     )) solver->Upwind = Euler1DUpwindRF;
  else if (!strcmp(physics->m_upw_choice,_LLF_    )) solver->Upwind = Euler1DUpwindLLF;
  ...
  if (!strcmp(solver->m_split_hyperbolic_flux,"yes")) {
    ...
    if      (!strcmp(physics->m_upw_choice,_ROE_ )) solver->UpwinddF = Euler1DUpwinddFRoe;
    else if (!strcmp(physics->m_upw_choice,_RF_  )) solver->UpwinddF = Euler1DUpwinddFRF;
  ...
  if      (!strcmp(physics->m_upw_choice,_LLF_ )) physics->SourceUpwind = Euler1DSourceUpwindLLF;
  else if (!strcmp(physics->m_upw_choice,_ROE_ )) physics->SourceUpwind = Euler1DSourceUpwindRoe;

  /* allocate array to hold the gravity field */
  int *dim    = solver->m_dim_local;
  int ghosts  = solver->m_ghosts;
  int size = 1; for (d=0; d<_MODEL_NDIMS_; d++) size *= (dim[d] + 2*ghosts);
  physics->m_grav_field = (double*) calloc (size, sizeof(double));
  physics->m_fast_jac   = (double*) calloc (size*_MODEL_NVARS_*_MODEL_NVARS_, sizeof(double));
  physics->m_solution   = (double*) calloc (size*_MODEL_NVARS_, sizeof(double));
