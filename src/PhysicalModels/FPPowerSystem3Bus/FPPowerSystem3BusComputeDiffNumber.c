                                         ...
                                         )
{
  ...
  while (!done) {
    ...
    FPPowerSystem3BusDissipationFunction(0,0,params,a_t,dissp);

    double local_diff[ndims];
    local_diff[0] = absolute(dissp[0*ndims+0]) * a_dt * dxinv[0] * dxinv[0];
    local_diff[1] = absolute(dissp[1*ndims+1]) * a_dt * dxinv[1] * dxinv[1];
    local_diff[2] = absolute(dissp[2*ndims+2]) * a_dt * dxinv[2] * dxinv[2];
    local_diff[3] = absolute(dissp[3*ndims+3]) * a_dt * dxinv[3] * dxinv[3];
