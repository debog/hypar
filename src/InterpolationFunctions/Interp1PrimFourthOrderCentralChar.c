/*! @file Interp1PrimFourthOrderCentralChar.c
    @author Debojyoti Ghosh
    @brief Characteristic-based fourth order central scheme
*/
...
/*! @brief 4th order central reconstruction (characteristic-based) on a uniform grid

    Computes the interpolated values of the first primitive of a function \f${\bf f}\left({\bf u}\right)\f$
    at the interfaces from the cell-centered values of the function using the 4th order central scheme on a
    ...
    \f}
    where \f$x\f$ is the spatial coordinate along the dimension of the interpolation. This function computes the 4th order central numerical approximation \f$\hat{\bf f}_{j+1/2} \approx {\bf h}_{j+1/2}\f$ as:
    \f{equation}{
      \hat{\alpha}^k_{j+1/2} = -\frac{1}{12} {\alpha}^k_{j-1} + \frac{7}{12} {\alpha}^k_{j} + \frac{7}{12} {\alpha}^k_{j+1} - \frac{1}{12} {\alpha}^k_{j+2},
    \f}
    where
    \f{equation}{
 ...
 */
int Interp1PrimFourthOrderCentralChar(
...
#pragma omp parallel for schedule(auto) default(shared) private(i,k,v,R,L,uavg,fchar,index_outer,indexC,indexI)
  for (i=0; i<N_outer; i++) {
    ...
    for (indexI[dir] = 0; indexI[dir] < dim[dir]+1; indexI[dir]++) {
      ...
      int pLL, pL, pR, pRR; /* 1D index of the left and right cells */
      indexC[dir] = indexI[dir]-2; _ArrayIndex1D_(ndims,dim,indexC,ghosts,pLL);
      ...
      indexC[dir] = indexI[dir]+1; _ArrayIndex1D_(ndims,dim,indexC,ghosts,pRR);
      int p; /* 1D index of the interface */
      _ArrayIndex1D_(ndims,bounds_inter,indexI,0,p);

      /* find averaged state at this interface */
      IERR solver->AveragingFunction(uavg,&u[nvars*pL],&u[nvars*pR],solver->m_physics); CHECKERR(ierr);
      ...
      for (v = 0; v < nvars; v++) {
        ...
        double fcLL = 0, fcL = 0, fcR = 0, fcRR = 0;
        for (k = 0; k < nvars; k++) {
          fcLL += L[v*nvars+k] * fC[pLL*nvars+k];
          fcL  += L[v*nvars+k] * fC[pL *nvars+k];
          fcR  += L[v*nvars+k] * fC[pR *nvars+k];
          fcRR += L[v*nvars+k] * fC[pRR*nvars+k];
        }
