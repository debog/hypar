...
/*! @brief 5th order hybrid compact-WENO reconstruction (characteristic-based) on a uniform grid
    ...
    \f}
    which is the \f$k\f$-th characteristic quantity, and \f${\bf l}_k\f$ is the \f$k\f$-th left eigenvector, \f${\bf r}_k\f$ is the \f$k\f$-th right eigenvector, and \f$n\f$ is #HyPar::m_nvars. The block tridiagonal system is solved using BlockTridiagLU() (see also #TridiagLU, tridiagLU.h). The final interpolated function is computed from the interpolated characteristic quantities as:
    \f{equation}{
                                   ...
                                   )
{
  ...
  static const double s_one_half           = 1.0/2.0;
  ...
  for (sys=0; sys<Nsys; sys++) {
    ...
    for (indexI[dir] = 0; indexI[dir] < dim[dir]+1; indexI[dir]++) {
      ...
      for (v=0; v<nvars; v++)  {
        ...
        if (upw > 0) {
          for (k=0; k<nvars; k++) {
            A[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (s_one_half*sigma)  * L[v*nvars+k];
            B[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (1.0)             * L[v*nvars+k];
            C[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (s_one_sixth*sigma) * L[v*nvars+k];
          }
        } else {
          for (k=0; k<nvars; k++) {
            C[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (s_one_half*sigma)  * L[v*nvars+k];
            B[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (1.0)             * L[v*nvars+k];
            A[(Nsys*indexI[dir]+sys)*nvars*nvars+v*nvars+k] = (s_one_sixth*sigma) * L[v*nvars+k];
          }
        }
        double fWENO, fCompact;
        fWENO    = w1*f1 + w2*f2 + w3*f3;
        fCompact = s_one_sixth * (s_one_third*fm2 + 19.0*s_one_third*fm1 + 10.0*s_one_third*fp1);
