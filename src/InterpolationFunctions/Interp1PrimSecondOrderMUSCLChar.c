/*! @file Interp1PrimSecondOrderMUSCLChar.c
    @author Debojyoti Ghosh
    @brief Characteristic-based 2nd-order MUSCL scheme
*/
...
/*! @brief 2nd order MUSCL scheme (characteristic-based) on a uniform grid
    ...
    using the 2nd order MUSCL scheme as follows:
    \f{equation}{
      \hat{\alpha}^k_{j+1/2} = {\alpha}^k_{j} + \frac{1}{2} \phi\left(r_j\right) \left[{\alpha}^k_{j+1}-{\alpha}^k_{j}\right]
    \f}
    where
    \f{equation}{
      r_j = \left( \alpha^k_j - \alpha^k_{j-1} \right) / \left( \alpha^k_{j+1} - \alpha^k_j \right),
    \f}
    \f$\phi\left(r\right)\f$ is a limiter (minmod, mc, generalized minmod, etc.), and
    \f{equation}{
 ...
 */
int Interp1PrimSecondOrderMUSCLChar (
