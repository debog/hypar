  ...
  for (int ns = 0; ns < nsims; ns++) {
    ...
    for (int n = 0; n < npoints; n++) {
      ...
      for (int dir = 0; dir < ndims; dir++) {
        ...
        solver->JFunction(values,(u+nvars*p),solver->physics,dir,0);
        ...
        if (pgL >= 0) {
          ...
          solver->JFunction(values,(u+nvars*pL),solver->physics,dir,1);
          _ArrayScale1D_(values,(-dxinv*iblank),(nvars*nvars));
          MatSetValues(Pmat,nvars,rows,nvars,cols,values,ADD_VALUES);
        }
        ...
        if (pgR >= 0) {
          ...
          solver->JFunction(values,(u+nvars*pR),solver->physics,dir,-1);
          _ArrayScale1D_(values,(-dxinv*iblank),(nvars*nvars));
          MatSetValues(Pmat,nvars,rows,nvars,cols,values,ADD_VALUES);
        }
      }
